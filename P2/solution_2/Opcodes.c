...
extern struct Semaphore Doorman;
...
int ExecuteProc(struct PCB *Current)
	...
	*/
	...
	/*Prints out the value of the Program Registers*/
	void PrintRegs()
	{int i, j ;
	 printf("\n") ;
	...
	//print all semaphore count and queues, if not null

	void printSem() {
		struct PCB * tmp;
		printf("\nDoorman count: %d ", Doorman.count);
		if (Doorman.SemQ != NULL) {
			tmp = Doorman.SemQ;
			while (tmp != NULL) {
				printf("Doorman Q: %d ", tmp->PID);
				tmp = tmp->Next_PCB;
			}
		}
		printf("\n");
		for(i = 0; i < 5; i++) {
			printf("S[%d] count: %d ", i, Forks[i].count);
	...
	int OS_Trap(char *IR, struct PCB *Current)
	{
		...
		switch (call) {
			case 0: semReg = ParseOp2Reg(IR);
				...
				if (semType == 0) {
					printf("Wait call: PID %d on Fork semaphore %d\n", Current->PID, ACC);
					return Wait(Current, &Forks[ACC]);
				}
				else if (semType == 1) {
					printf("Wait call: PID %d on Doorman semaphore\n", Current->PID);
			...
			case 1: semReg = ParseOp2Reg(IR);
				semType = RRegs[semReg];
				if (semType == 0) {
					printf("Signal call: PID %d on Fork semaphore %d\n", Current->PID, ACC);
					return Signal(&Forks[ACC]);
				}
				else if (semType == 1) {
					printf("Signal call: PID %d on Doorman semaphore\n", Current->PID);
	...
	*/
	...
	void OP37(char *IR)
	{
		...
		printf("Opcode = 37. Modulo Operation\n");
