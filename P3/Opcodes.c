...
int ExecuteProc(struct PCB *Current)
{
	...
	while(Current->IC > j && !Done)  //until slice completes or halt instruction called
	{
		/* This is implementing the Instruction Cycle */
		/*First, fetch instruction */
		address = Translate_Address(PC);
		for(i = 0; i < 6; i++)
			IR[i] = memory[address][i];	//adjust to baseReg
		...
		/*Now execute instruction and increment PC (unless Branch) */
			...
			case 0:    OP0(IR) ; PC++; break ;
	      		...
	      		case 99: printf("ALL DONE. Memory Dump.\n") ; Done = 1 ; break;
...
int Translate_Address (int Logical_Address)
{
	int mem_page = Page_Table[Current->PID][Logical_Address / 10]; //get memory page from from page table
	int physical_address = (mem_page * 10) + (Logical_Address % 10); //convert to physical address
	
	printf("\n******Memory Access******\n");
	printf("Logical Address: %d, Translated Address: %d\n\n", Logical_Address, physical_address);
...
void StoreData(int Memory_Location, int Value)
{
	...
	printf("Value stored at memory location %d: %d\n", Memory_Location, Value);
}
